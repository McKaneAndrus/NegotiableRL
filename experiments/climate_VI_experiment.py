    ...
    try:
        ...
        while diffs < num_agents * (round(nT ** nR * sparsity) + 1):
            mdps = [ClimateControlEnv(num_rooms=nR, num_temps=nT, sparsity=sparsity, max_steps=ms) for _ in
                    range(num_agents)]
            q_tuples,_,_ = SparseNocAlg.find_diffs(mdps)
            diffs = len(q_tuples)
        for mdp in mdps:
            mdp.seed(0)
        ...
        for i in obs_range:
            ...
            try:
                start = timer()
                sparse_nocs += [SparseNocAlg(climate_game, ms, obs_horizon=i, keep_values=True)]
                end = timer()
        ...
        best = (0, float("-inf"))
        _, ts = test_alg_settings(POMDPlite(climate_game, ms, beta=max(betas), keep_values=True), 2, save_times=True)
        ...
        for beta in betas:
            score = test_alg_settings(POMDPlite(climate_game, ms, beta=beta, keep_values=True), plite_breadth).mean()
            if score > best[0]:
                best = (score, beta)
        full_plite_test, full_plite_times = test_alg_settings(POMDPlite(climate_game, ms, beta=best[1], keep_values=True),
