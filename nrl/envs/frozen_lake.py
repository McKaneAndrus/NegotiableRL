...
class PartiallyFrozenLakeEnvMultigoal(discrete_env.DiscreteEnv):
    """
    Winter is here. You and your friends were tossing around a frisbee at the park
    when you made a wild throw that left the frisbee out in the middle of the lake.
    The water is mostly frozen, but there are a few holes where the ice has melted.
    ...
    def __init__(self, desc=None, map_name="8x8_multigoal",is_slippery=True,partial_slip=0.2,goal=1):
        assert str(goal) in '12'
        if desc is None and map_name is None:
            raise ValueError('Must provide either desc or map_name')
        elif desc is None:
            desc = PARTIAL_MAPS[map_name]
        self.desc = desc = np.asarray(desc,dtype='c')
        self.nrow, self.ncol = nrow, ncol = desc.shape
        self.trans_tuples = []
        self.trans_to_PR = {}
        ...
        # goal_check = [bytes(line,'utf-8').index(goalchar) if goalchar in bytes(line,'utf-8') else -1 for line in desc]
        # goal_index = 0
        # for i, index in enumerate(goal_check):
        #     if index > -1:
        #         goal_index = index + i * ncol
        #         break
        # self.single_step_rewards = np.zeros((nrow,ncol))
        # self.single_step_rewards[goal_index] = 1
                ...
                for a in range(4):
                    ...
                    if letter in b'12GH':
                        ...
                        for b in [(a-1)%4, a, (a+1)%4]:
                            ...
                            self.trans_tuples += [(s,a,newstate)]
                            self.trans_to_PR[(s,a,newstate)] = (1 - 2 * partial_slip if b==a else partial_slip,rew)
        ...
        super(PartiallyFrozenLakeEnvMultigoal, self).__init__(nS, nA, P, isd)

    def get_trans_PR(self, trans):
        return self.trans_to_PR[trans]
...
class ParamFrozenLakeEnvMultigoal(discrete_env.DiscreteEnv):
    """
    Winter is here. You and your friends were tossing around a frisbee at the park
    when you made a wild throw that left the frisbee out in the middle of the lake.
    The water is mostly frozen, but there are a few holes where the ice has melted.
    ...
    def __init__(self, diffs=1, desc=None, map_name="8x8_multigoal",is_slippery=True,partial_slip=0.2,rewards=[0,1]):
        ...
        if type(diffs) == int:
            ...
            nP = dict(zip(*np.unique(self.desc, return_counts=True)))[b'P']
            if diffs > nP * 4:
                print("Number of differences too great for chosen map. Number was reduced.")
                diffs = nP * 4
            diff_select = np.array(([True] * diffs) + ([False] * (nP*4-diffs)))
            rand.shuffle(diff_select)
            diff_select = list(diff_select)
        else:
            if diffs.shape != (nS,nA):
                print("Incorrect shape of difference matrix.")
                return
            Ps = np.where(desc.flatten()  == b'P')[0]
            diff_select = list(diffs[Ps].flatten())
        ...
        rewards += [0,0,0,0]
        grid_rewards = {char:rewards[i] for i,char in enumerate([b'1',b'2',b'S',b'H',b'F',b'P'])}
            ...
            for col in range(ncol):
                ...
                for a in range(4):
                    ...
                    if letter in b'12GH':
                        li.append((1.0, s, 0, True))
                        self.sinks += [s]
                    elif letter in b'P' and diff_select.pop(0):
                        self.q_tuples += [(s,a)]
                        for b in [(a-1)%4, a, (a+1)%4]:
                            newrow, newcol = inc(row, col, b)
                            newstate = to_s(newrow, newcol)
        ...
        super(ParamFrozenLakeEnvMultigoal, self).__init__(nS, nA, P, isd)
